...
def test_build_messages():
    client = OpenAI(api_key="ss")

    print("\n==== Test: No reason, no prompt_template ====")
    detector1 = GemmaQuestionDetector(client, model="gemma-3")
    try:
    ...
    detector2 = GemmaQuestionDetector(
        client, model="gemma-3", prompt_template="Please check if this is a question."
    )
    try:
    ...
    detector3 = GemmaQuestionDetector(
        client, model="gemma-3", prompt_template="Check it carefully."
    )
    try:
        msgs3 = detector3._build_messages(
            "Is Python your favorite language?",
            reason="I need to be sure this is a question.",
        )
